...
## break and a depth marker worth a large score chunk and guaranteed XP. It
## scales per appearance so it stays a threat as the endless ramp climbs.
##
## Determinism: everything here runs on fixed cadences. The burst rotation
## advances by a constant per burst rather than being randomised, because the
## boss fires on player-dependent timing and any RNG draw here would desync the
## shared spawn stream.
...
const RADIUS := 46.0
const BURST_INTERVAL := 3.4
const BURST_SHOTS := 8
const BURST_ROTATION := 0.21          # radians added per burst, so gaps move
const COLOR := Color(1.0, 0.30, 0.55)
...
var _player: Node2D
var _flash := 0.0
var _burst_cooldown := BURST_INTERVAL
var _bursts_fired := 0
var _spin := 0.0
...
func _fire(delta: float) -> void:
	...
	var base_angle := float(_bursts_fired) * BURST_ROTATION
	_bursts_fired += 1
	for i in BURST_SHOTS:
		var angle := base_angle + TAU * float(i) / float(BURST_SHOTS)
		var shot := ENEMY_SHOT_SCENE.instantiate()
		shot.position = position + Vector2(RADIUS * 0.8, 0.0).rotated(angle)
		shot.setup(Vector2.RIGHT.rotated(angle), contact_damage * 0.35)
