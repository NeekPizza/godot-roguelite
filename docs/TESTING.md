...
| `--meta-file=<name>` | Redirects the meta profile to `user://<name>` so a test can exercise REAL persistence (records, lifetime totals) without touching a player's actual profile. Mirrors `--save-file`. In `TEST_HOOK_ARGS`: a hook that writes the progression file has no business near a ranked submission. |
