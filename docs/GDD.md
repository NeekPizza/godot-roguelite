...
- **Music:** one 85.7 s synthwave loop, **generated by `tools/gen_music.py`** —
  ...
  dubstep-leaning low end: a sine sub under everything plus a tempo-synced
  Reese wobble bass (4/8/16 wobbles per bar) in the heavy sections. Plays at
  −17 dB so it sits under the SFX.
- **SFX:** 7 retro/sfxr-style sounds — shoot, hit, enemy death, XP pickup,
  level-up, player hurt, run over. **Generated by `tools/gen_sfx.py`**, our own
